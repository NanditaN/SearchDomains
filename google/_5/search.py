...
#This method creates a dictionary out of the CSV sample file
...
#Get the final list of URLs for a company name
def parent_input(company_name, no_of_results):
    list_1 = get_input(company_name, no_of_results)

    print("Closest matches obtained from Google: ")
    print(list_1);
    ...
    for url in list_1:
        ...
        """
        The following checks are disabled to yeild better results.
        If these checks are enabled, the list that gets populated ,might miss out on the expected URL value
        due to absence of title or string of copyright
        """
        ...
        #if check(company_name, title.lower()) or check(company_name, copyright_1.lower()) or check(company_name, copyright_2.lower()):
        #    final_list_f.append(strip_to_website(url))
    #print(final_list_f)
    #return final_list_f
    return list_1
...
no_of_results = 20
model_csv = 'C:/Users/abc/Desktop/siftery-data.csv';
random_sample_size = 10

#Gets the dictionary containing 5k Key-value pair
dictionary = get_model_data(model_csv);
key_list = dictionary.keys();
selected_key_list = random.sample(key_list, random_sample_size);

final_success_count = 0

#Checking if the output generated is the expected one or not
for k in selected_key_list:
    print("Company name: " + k)
    print("Company URL: " + dictionary[k].strip())
    
    final_list = parent_input(k.strip(), no_of_results)
    ...
    for link in final_list:
        if dictionary[k].strip().lower() in link.lower():
            #print(link)
            success_count += 1;
...
print("Success rate: " + str(final_success_count * 100 / random_sample_size) + "%")
